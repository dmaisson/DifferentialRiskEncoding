function diff = mFRdiff_SafeRisky(in,probs)

% setup variable
for iJ = 1:length(in.safe.Ep1)
    x = nanmean(in.safe.Ep1{iJ}.psth,2);
    safe(iJ,1) = nanmean(x);
    clear x;
    x = nanmean(in.eqlow.Ep1{iJ}.psth,2);
    low(iJ,1) = nanmean(x);
    clear x;
    x = nanmean(in.eqhigh.Ep1{iJ}.psth,2);
    high(iJ,1) = nanmean(x);
    clear x;
    for iK = 1:length(probs)
        x = nanmean(in.low.Ep1{iK}.cell{iJ}.psth,2);
        mFR.low(iJ,iK) = nanmean(x);
        clear x;
        x = nanmean(in.high.Ep1{iK}.cell{iJ}.psth,2);
